...
FVector UProwerMovementComponent::GetImpartedMovementBaseVelocity() const
{
...
void UProwerMovementComponent::JumpOff(AActor* MovementBaseActor)
{
...
FVector UProwerMovementComponent::GetBestDirectionOffActor(AActor* BaseActor) const
{
...
void UProwerMovementComponent::SetDefaultMovementMode()
{
...
void UProwerMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	if (!HasValidData())
	{
		return;
...
void UProwerMovementComponent::PerformAirControlForPathFollowing(FVector Direction, float ZDiff)
{
...
void UProwerMovementComponent::PerformAirControlForPathFollowingEx(const FVector& MoveVelocity, const FVector& GravDir)
{
...
FVector UProwerMovementComponent::ConstrainAnimRootMotionVelocity(const FVector& RootMotionVelocity, const FVector& CurrentVelocity) const
{
...
void UProwerMovementComponent::MaybeUpdateBasedMovement(float DeltaSeconds)
{
...
void UProwerMovementComponent::UpdateBasedMovement(float DeltaSeconds)
{
	if (!HasValidData())
	{
		return;
...
void UProwerMovementComponent::UpdateBasedRotation(FRotator& FinalRotation, const FRotator& ReducedRotation)
{
...
void UProwerMovementComponent::Crouch(bool bClientSimulation)
{
	if (!HasValidData())
	{
		return;
	...
	const float ClampedCrouchedHalfHeight = FMath::Max3(0.0f, OldUnscaledRadius, GetCrouchedHalfHeight());
	...
	if (!bClientSimulation)
	{
		...
		{
			FCollisionQueryParams CapsuleParams(MovementComponentStatics::CrouchTraceName, false, CharacterOwner);
...
void UProwerMovementComponent::UnCrouch(bool bClientSimulation)
{
	if (!HasValidData())
	{
		return;
...
float UProwerMovementComponent::SlideAlongSurface(const FVector& Delta, float Time, const FVector& InNormal, FHitResult& Hit, bool bHandleImpact)
{
	if (!Hit.bBlockingHit)
	{
		return 0.0f;
...
void UProwerMovementComponent::TwoWallAdjust(FVector& Delta, const FHitResult& Hit, const FVector& OldHitNormal) const
{
...
FVector UProwerMovementComponent::HandleSlopeBoosting(const FVector& SlideResult, const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit) const
{
	const FVector CapsuleUp = GetComponentAxisZ();
...
float UProwerMovementComponent::ImmersionDepth() const
{
	float Depth = 0.0f;
	...
	if (CharacterOwner && GetPhysicsVolume()->bWaterVolume)
	{
		...
		else
		{
			...
			if (VolumeBrushComp)
			{
				...
				FCollisionQueryParams NewTraceParams(MovementComponentStatics::ImmersionDepthName, true);
...
void UProwerMovementComponent::RequestDirectMove(const FVector& MoveVelocity, bool bForceMaxSpeed)
{
...
float UProwerMovementComponent::GetMaxJumpHeight() const
{
...
void UProwerMovementComponent::ApplyRootMotionToVelocityOVERRIDEN(float deltaTime)
{
...
void UProwerMovementComponent::PhysSwimming(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
...
void UProwerMovementComponent::StartSwimmingOVERRIDEN(FVector OldLocation, FVector OldVelocity, float timeTick, float remainingTime, int32 Iterations)
{
...
FVector UProwerMovementComponent::GetFallingLateralAcceleration(float DeltaTime)
{
	return GetFallingLateralAccelerationEx(DeltaTime, GetGravityDirection(true));
}

FVector UProwerMovementComponent::GetFallingLateralAccelerationEx(float DeltaTime, const FVector& GravDir) const
{
...
FVector UProwerMovementComponent::GetAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration)
{
	return GetAirControlEx(DeltaTime, TickAirControl, FallAcceleration, GetGravityDirection(true));
}

FVector UProwerMovementComponent::GetAirControlEx(float DeltaTime, float TickAirControl, const FVector& FallAcceleration, const FVector& GravDir) const
{
	// Boost.
...
float UProwerMovementComponent::BoostAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration)
{
	return BoostAirControlEx(DeltaTime, TickAirControl, FallAcceleration, GetGravityDirection(true));
}

float UProwerMovementComponent::BoostAirControlEx(float DeltaTime, float TickAirControl, const FVector& FallAcceleration, const FVector& GravDir) const
{
...
FVector UProwerMovementComponent::LimitAirControl(float DeltaTime, const FVector& FallAcceleration, const FHitResult& HitResult, bool bCheckForValidLandingSpot)
{
	return LimitAirControlEx(DeltaTime, FallAcceleration, HitResult, GetGravityDirection(true), bCheckForValidLandingSpot);
}

FVector UProwerMovementComponent::LimitAirControlEx(float DeltaTime, const FVector& FallAcceleration, const FHitResult& HitResult, const FVector& GravDir, bool bCheckForValidLandingSpot) const
{
...
bool UProwerMovementComponent::CheckLedgeDirection(const FVector& OldLocation, const FVector& SideStep, const FVector& GravDir) const
{
	const FVector SideDest = OldLocation + SideStep;
	const FQuat PawnRotation = UpdatedComponent->GetComponentQuat();
	FCollisionQueryParams CapsuleParams(MovementComponentStatics::CheckLedgeDirectionName, false, CharacterOwner);
	FCollisionResponseParams ResponseParam;
	InitCollisionParams(CapsuleParams, ResponseParam);
...
FVector UProwerMovementComponent::GetLedgeMove(const FVector& OldLocation, const FVector& Delta, const FVector& GravDir) const
{
...
FVector UProwerMovementComponent::ComputeGroundMovementDelta(const FVector& Delta, const FHitResult& RampHit, const bool bHitFromLineTrace) const
{
	const FVector CapsuleUp = GetComponentAxisZ();
	return ComputeGroundMovementDeltaEx(FVector::VectorPlaneProject(Delta, CapsuleUp), CapsuleUp, RampHit, bHitFromLineTrace);
}

FVector UProwerMovementComponent::ComputeGroundMovementDeltaEx(const FVector& Delta, const FVector& DeltaPlaneNormal, const FHitResult& RampHit, const bool bHitFromLineTrace) const
{
...
void UProwerMovementComponent::MoveAlongFloor(const FVector& InVelocity, float DeltaSeconds, FStepDownResult* OutStepDownResult)
{
...
void UProwerMovementComponent::MaintainHorizontalGroundVelocity()
{
	if (bMaintainHorizontalGroundVelocity)
	{
...
void UProwerMovementComponent::AdjustFloorHeight()
{
...
void UProwerMovementComponent::SetPostLandedPhysics(const FHitResult& Hit)
{
	if (CharacterOwner)
	{
		if (CanEverSwim() && IsInWater())
...
void UProwerMovementComponent::OnTeleported()
{
	if (!HasValidData())
	{
		return;
...
void UProwerMovementComponent::PhysicsRotation(float DeltaTime)
{
...
void UProwerMovementComponent::PhysicsVolumeChanged(class APhysicsVolume* NewVolume)
{
	if (!HasValidData())
	{
		return;
...
bool UProwerMovementComponent::ShouldJumpOutOfWater(FVector& JumpDir)
{
	return ShouldJumpOutOfWaterEx(JumpDir, GetGravityDirection(true));
}

bool UProwerMovementComponent::ShouldJumpOutOfWaterEx(FVector& JumpDir, const FVector& GravDir)
{
...
bool UProwerMovementComponent::CheckWaterJump(FVector CheckPoint, FVector& WallNormal)
{
	return CheckWaterJumpEx(CheckPoint, GetGravityDirection(true), WallNormal);
}

bool UProwerMovementComponent::CheckWaterJumpEx(FVector CheckPoint, const FVector& GravDir, FVector& WallNormal)
{
	if (!HasValidData())
	{
		return false;
	...
	FCollisionQueryParams CapsuleParams(MovementComponentStatics::CheckWaterJumpName, false, CharacterOwner);
	const FCollisionShape CapsuleShape = GetPawnCapsuleCollisionShape(SHRINK_None);
	const ECollisionChannel CollisionChannel = UpdatedComponent->GetCollisionObjectType();
	FCollisionResponseParams ResponseParam;
	InitCollisionParams(CapsuleParams, ResponseParam);
	...
	if (bHit && !Cast<APawn>(HitInfo.GetActor()))
	{
		...
		FCollisionQueryParams LineParams(MovementComponentStatics::CheckWaterJumpName, true, CharacterOwner);
...
void UProwerMovementComponent::MoveSmooth(const FVector& InVelocity, const float DeltaSeconds, FStepDownResult* OutStepDownResult)
{
	if (!HasValidData())
	{
		return;
...
bool UProwerMovementComponent::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	return IsWithinEdgeToleranceEx(CapsuleLocation, GetComponentAxisZ() * -1.0f, CapsuleRadius, TestImpactPoint);
}

bool UProwerMovementComponent::IsWithinEdgeToleranceEx(const FVector& CapsuleLocation, const FVector& CapsuleDown, const float CapsuleRadius, const FVector& TestImpactPoint) const
{
...
void UProwerMovementComponent::ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance, float SweepDistance, FFindFloorResult& OutFloorResult, float SweepRadius, const FHitResult* DownwardSweepResult) const
{
	...
	if (!bSkipSweep && SweepDistance > 0.0f && SweepRadius > 0.0f)
	{
		...
		QueryParams.TraceTag = MovementComponentStatics::ComputeFloorDistName;
	...
	if (LineDistance > 0.0f)
	{
		...
		QueryParams.TraceTag = MovementComponentStatics::FloorLineTraceName;
...
bool UProwerMovementComponent::FloorSweepTest(FHitResult& OutHit, const FVector& Start, const FVector& End, ECollisionChannel TraceChannel,
...
bool UProwerMovementComponent::ShouldCheckForValidLandingSpot(float DeltaTime, const FVector& Delta, const FHitResult& Hit) const
{
	const FVector CapsuleUp = GetComponentAxisZ();
...
bool UProwerMovementComponent::ShouldComputePerchResult(const FHitResult& InHit, bool bCheckRadius) const
{
...
bool UProwerMovementComponent::ComputePerchResult(const float TestRadius, const FHitResult& InHit, const float InMaxFloorDist, FFindFloorResult& OutPerchFloorResult) const
{
...
bool UProwerMovementComponent::StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult& InHit, struct FStepDownResult* OutStepDownResult)
{
...
void UProwerMovementComponent::HandleImpact(const FHitResult& Impact, float TimeSlice, const FVector& MoveDelta)
{
	if (CharacterOwner)
	{
...
void UProwerMovementComponent::ApplyImpactPhysicsForces(const FHitResult& Impact, const FVector& ImpactAcceleration, const FVector& ImpactVelocity)
{
...
void UProwerMovementComponent::DisplayDebug(UCanvas* Canvas, const FDebugDisplayInfo& DebugDisplay, float& YL, float& YPos)
{
...
FVector UProwerMovementComponent::ConstrainInputAcceleration(const FVector& InputAcceleration) const
{
...
void UProwerMovementComponent::CapsuleTouched(UPrimitiveComponent* OverlappedComp, AActor* Other, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (!bEnablePhysicsInteraction)
	{
		return;
...
void UProwerMovementComponent::ApplyDownwardForce(float DeltaSeconds)
{
...
void UProwerMovementComponent::CalcVelocity(float DeltaTime, float Friction, bool bFluid, float BrakingDeceleration)
{
...
void UProwerMovementComponent::ApplyRepulsionForce(float DeltaSeconds)
{
...
void UProwerMovementComponent::ApplyAccumulatedForces(float DeltaSeconds)
{
...
FVector UProwerMovementComponent::GetGravity() const
{
...
FVector UProwerMovementComponent::GetGravityDirection(bool bAvoidZeroGravity) const
{
...
float UProwerMovementComponent::GetGravityMagnitude() const
{
	return FMath::Abs(GetGravityZ());
}

void UProwerMovementComponent::SetGravityDirection(const FVector& NewGravityDirection)
{
	SetCustomGravityDirection(NewGravityDirection.GetSafeNormal());
}

FORCEINLINE void UProwerMovementComponent::SetCustomGravityDirection(const FVector& NewCustomGravityDirection)
{
...
void UProwerMovementComponent::UpdateGravity(float DeltaTime)
{
...
FRotator UProwerMovementComponent::ConstrainComponentRotation(const FRotator& Rotation) const
{
...
FORCEINLINE FVector UProwerMovementComponent::GetComponentAxisX() const
{
...
FORCEINLINE FVector UProwerMovementComponent::GetComponentAxisZ() const
{
...
FVector UProwerMovementComponent::GetComponentDesiredAxisZ() const
{
